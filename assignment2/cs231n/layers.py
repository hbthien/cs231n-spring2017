...
def affine_backward(dout, cache):
    ...
    dx = dout.dot(w.T).reshape(x.shape) #dx=w*dout, (N,M)x(M,D)=(N,D)
...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout*gamma
    d_sample_var = np.sum(dx_hat * (x-sample_mean) * (-0.5) * (sample_var + eps) ** (-1.5), axis=0)
    d_sample_mean = np.sum(dx_hat * (-1) / np.sqrt(sample_var + eps), axis=0) + d_sample_var * np.mean((-2) * (x - sample_mean), axis=0)
    dx = dx_hat / np.sqrt(sample_var + eps) + d_sample_var * 2 * (x - sample_mean) / m + d_sample_mean / m
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    padded_x = np.pad(x, [(0, 0), (0, 0), (P, P), (P, P)], mode='constant') #Number of values padded to the edges of each axis. ((before_1, after_1)...), default constant=0
